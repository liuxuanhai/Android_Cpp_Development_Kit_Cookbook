...
jint naMain(JNIEnv *pEnv, jobject pObj, jobjectArray pArgv, jobject pGameViewObj) {
	...
	jclass jMainActCls = (*pEnv)->GetObjectClass(pEnv, pObj);
	cachedMainActObj = (*pEnv)->NewGlobalRef(pEnv, pObj);
	...
	playSoundMID = (*pEnv)->GetMethodID(pEnv, jMainActCls, "playSound", "(I)V");
	if (NULL == playSoundMID) {
		LOGE(1, "error finding method playSound");
		return -1;
	}

	playMusicMID = (*pEnv)->GetMethodID(pEnv, jMainActCls, "playMusic", "(I)V");
	if (NULL == playMusicMID) {
		LOGE(1, "error finding method playMusic");
		return -1;
	}
	...
	(*pEnv)->DeleteGlobalRef(pEnv, cachedMainActObj);
...
//call the Java method to play background music
void jni_playSound(int idx) {
	JNIEnv *pEnv;
	(*cachedJvm)->AttachCurrentThread(cachedJvm, &pEnv, NULL);
	(*pEnv)->CallVoidMethod(pEnv, cachedMainActObj, playSoundMID, idx);
}

//call the Java method to play sound
void jni_playMusic(int idx) {
	JNIEnv *pEnv;
	(*cachedJvm)->AttachCurrentThread(cachedJvm, &pEnv, NULL);
	(*pEnv)->CallVoidMethod(pEnv, cachedMainActObj, playMusicMID, idx);
}
