...
int OpenWrite(const char *fn)
{
	...
	fp = open(fpath, O_CREAT|O_WRONLY|O_TRUNC|O_BINARY, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
	return fp;
}
...
int OpenWriteAppend(const char *fn)
{
	...
	char fpath[200];
	sprintf(fpath, "%s%s", datadir, fn);
	fp = open(fpath, O_CREAT|O_WRONLY|O_BINARY, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
	return fp;
}
